...
EXPECT = {
    ...
    # 台本03(妊娠・創立記念式)で実機確認した語
    #   単独の「白石」は naist-jdic で ハクセキ に落ちることがある。句点を打つと シライシ。
    #   「亮」は既定で アキラ。台本では リョウ なので辞書登録側で直す(EXPECTには入れない)。
    "白石":"シライシ","千尋":"チヒロ","郡司":"グンジ","柏木":"カシワギ","相原":"アイハラ",
    "戸田":"トダ","前田":"マエダ","川本":"カワモト","佳代":"カヨ","佳澄":"カスミ",
    "肌着":"ハダギ","領収":"リョウシュウ","棚卸し":"タナオロシ","産業":"サンギョウ",
    "頸管":"ケイカン","腹帯":"ハラオビ","戌":"イヌ","盾":"タテ","欠勤":"ケッキン",
}
# 固有名詞(1アクセント句にまとまっているべきもの)
NAMES = ["中村結衣","中村健一","神崎涼介","神崎政子","鷹野誠一","鷹野物産","東洋通商","みやこ食堂",
         "森口菜穂","森口辰次","森口和夫","大西泰然","堤芳江","妙相寺",
         "白石千尋","白石亮","郡司里子","川本佳代","マルセイストア"]
...
def main(path):
    ...
    PHRASE = {
        ...
        "十分の一": "ジュウブンノイチ",
        # 台本10で三稿まで素通りした。「一食」は イッ・ショク で通るのに
        # 「一食分」は 一/食分 で切れて イチ・ショクブン(食分=日食の食分)になる。
        "一食分": "イッショクブン", "一食分の量": "イッショクブンノリョウ",
    }
    phr = []
    for ln, s2 in lines:
...
def dump(path):
    """全行を実機に通して、カナを印刷する(期待リストを使わない)。

    `main()` の ✅ は「期待リストに載せた語が期待どおりだった」以上の意味を持ちません。
    オギボサン・ミノコ・コンナバナシ・アノヒト は、誰も期待リストに書かなかったから通りました。
    **自分が書いていない行も含めて、ここに出るカナを一行ずつ目で読んでください。**
    """
    lines = load(path)
    print(f"読みダンプ: {path}  ({len(lines)}行)  engine=open_jtalk/naist-jdic")
    print("※ ✅ではなく、カナを読む。\n")
    for ln, s in lines:
        print(f"L{ln}  {s}")
        print(f"      → {''.join(r['pron'] for r in njd(s))}")
    return 0

if __name__ == "__main__":
    args = [a for a in sys.argv[1:] if not a.startswith("-")]
    path = args[0] if args else "01_披露宴_定食屋の親.md"
    sys.exit(dump(path) if "--dump" in sys.argv else main(path))
